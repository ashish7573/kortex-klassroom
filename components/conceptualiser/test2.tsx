...
const LeftHand = ({ count, onClick }: { count: number, onClick: () => void }) => {
  ...
  return (
    <svg viewBox="0 0 100 120" className="w-full h-full max-h-48 drop-shadow-xl cursor-pointer hover:scale-105 active:scale-95 transition-transform" onClick={onClick}>
      ...
      {/* Thumb (Right side) - Adjusted to come out more */}
      <rect x="65" y="65" width="22" height="14" rx="6" fill="#fcd34d" stroke="#d97706" strokeWidth="2" className="transition-all duration-300 origin-[65px_72px]" style={{ transform: open[4] ? 'rotate(-65deg) scale(1.1) translateY(-5px)' : 'rotate(0deg) translateX(-15px) scaleX(0.5)' }} />
...
const RightHand = ({ count, onClick }: { count: number, onClick: () => void }) => {
  ...
  return (
    <svg viewBox="0 0 100 120" className="w-full h-full max-h-48 drop-shadow-xl cursor-pointer hover:scale-105 active:scale-95 transition-transform" onClick={onClick}>
      {/* Thumb (Left side) - Adjusted to come out more */}
      <rect x="13" y="65" width="22" height="14" rx="6" fill="#fcd34d" stroke="#d97706" strokeWidth="2" className="transition-all duration-300 origin-[35px_72px]" style={{ transform: open[4] ? 'rotate(65deg) scale(1.1) translateY(-5px)' : 'rotate(0deg) translateX(15px) scaleX(0.5)' }} />
