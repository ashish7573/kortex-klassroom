...
// ============================================================================
const PlayerEngine = ({ playerId, wordPool, score, onScoreChange, isFlipped, isMultiplayer, isHD }: any) => {
    ...
    const loadNewWord = useCallback(() => {
        ...
        let newBubbles = [];
        const speedMult = isHD ? 1.5 : 2.0; // Hyper-Velocity for fast separation
        ...
        neededChars.forEach(char => {
            newBubbles.push({
                ...
                // Math ensures speed is NEVER 0. Always heavily ricocheting.
                vx: (Math.random() > 0.5 ? 1 : -1) * (0.8 + Math.random() * 0.8) * speedMult,
                vy: (Math.random() > 0.5 ? 1 : -1) * (0.8 + Math.random() * 0.8) * speedMult,
            });
        });

        const distractorCount = isHD ? 15 : 10; // Reduce DOM nodes in Perf mode
    ...
    // --- HIGH PERFORMANCE PHYSICS THROTTLE ---
    useEffect(() => {
        if (!targetWord) return;
        
        const targetFPS = isHD ? 60 : 24; // 24FPS Throttle for Smartboards
        ...
        const animate = (time: number) => {
            ...
            // Skip frame if rendering too fast
            if (deltaTime < frameInterval) return;
            
            // Delta-Time Compensation: Ensures letters move at the same physical speed even if frames drop
    ...
    return (
        <div className={`flex-1 flex flex-col w-full h-full relative rounded-3xl overflow-hidden shadow-inner touch-none ${isHD ? 'border-2 border-slate-200 bg-white' : 'border-4 border-slate-300 bg-slate-100'}`}>
            ...
            {/* ROTATED HUD */}
            <div className={`w-full bg-white border-b-4 border-${playerColor}-200 p-2 md:p-3 flex items-center justify-between gap-2 z-20 transition-transform duration-300 ${isHD ? 'shadow-md' : 'shadow-none border-b-8'} ${isFlipped ? 'rotate-180 lg:rotate-0' : ''}`}>
            ...
            <div className={`flex-1 relative w-full overflow-hidden ${isHD ? `bg-gradient-to-b from-${playerColor}-50 to-white shadow-[inset_0_0_20px_rgba(0,0,0,0.05)]` : `bg-slate-200 border-y-2 border-slate-300`}`}>
                {scoreFloat && (
                ...
                {bubbleList.map(bubble => (
                    <button
                        ...
                        style={{ willChange: 'top, left' }} // Hardware acceleration flag
...
export default function HindiWordCrush({ lesson, onComplete = () => {} }: any) {
    ...
    const [config, setConfig] = useState({ length: 2, players: 1, isHD: true }); // NEW: HD Toggle
    ...
    if (gameState === 'menu') {
        return (
            <div className="w-full h-full min-h-[500px] flex flex-col items-center justify-center p-4 md:p-6 bg-slate-50 rounded-3xl overflow-y-auto relative">
                <div className="text-center mb-6 md:mb-8 relative z-10 mt-4 md:mt-0">
                    ...
                    <p className="text-sm md:text-lg font-bold text-slate-500">Arcade Engine</p>
                </div>

                {/* GRAPHICS TOGGLE */}
                <div className="mb-6 flex justify-center relative z-20">
                   <div className="bg-slate-800/90 backdrop-blur-sm p-1.5 rounded-xl inline-flex border border-slate-700 shadow-inner">
                      <button 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 w-full max-w-3xl relative z-10 pb-4">
                    <div className="bg-white/80 backdrop-blur-md p-5 md:p-8 rounded-3xl border-2 border-slate-100 shadow-sm flex flex-col items-center text-center">
                        ...
                        <div className="flex flex-col gap-2 md:gap-3 w-full">
                            <button onClick={() => startGame(2, 1)} className="py-3 bg-white hover:bg-blue-50 border-b-4 border-slate-200 hover:border-blue-400 font-black text-slate-700 rounded-xl transition-all shadow-sm">2-Letter Words</button>
                            <button onClick={() => startGame(3, 1)} className="py-3 bg-white hover:bg-blue-50 border-b-4 border-slate-200 hover:border-blue-400 font-black text-slate-700 rounded-xl transition-all shadow-sm">3-Letter Words</button>
                            <button onClick={() => startGame(4, 1)} className="py-3 bg-white hover:bg-blue-50 border-b-4 border-slate-200 hover:border-blue-400 font-black text-slate-700 rounded-xl transition-all shadow-sm">4-Letter Words</button>
                        </div>
                    </div>
                    ...
                    {isMobile ? (
                        ...
                        <div className="bg-white/80 backdrop-blur-md p-5 md:p-8 rounded-3xl border-2 border-slate-100 shadow-sm flex flex-col items-center text-center">
                            ...
                            <div className="flex flex-col gap-2 md:gap-3 w-full">
                                <button onClick={() => startGame(2, 2)} className="py-3 bg-white hover:bg-rose-50 border-b-4 border-slate-200 hover:border-rose-400 font-black text-slate-700 rounded-xl transition-all shadow-sm">2-Letter Words</button>
                                <button onClick={() => startGame(3, 2)} className="py-3 bg-white hover:bg-rose-50 border-b-4 border-slate-200 hover:border-rose-400 font-black text-slate-700 rounded-xl transition-all shadow-sm">3-Letter Words</button>
                                <button onClick={() => startGame(4, 2)} className="py-3 bg-white hover:bg-rose-50 border-b-4 border-slate-200 hover:border-rose-400 font-black text-slate-700 rounded-xl transition-all shadow-sm">4-Letter Words</button>
                            </div>
                        </div>
                    )}
                </div>
    ...
    if (gameState === 'gameover') {
        ...
        return (
            <div className="w-full h-full min-h-[500px] flex flex-col items-center justify-center p-6 text-center bg-slate-50 rounded-3xl">
                ...
                {config.players === 2 && <p className="text-xl md:text-2xl font-bold text-indigo-500 mb-6 md:mb-8">{winner === 'Tie' ? "It's a Tie!" : `${winner} Wins!`}</p>}
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full max-w-sm">
                    ...
                    <button onClick={() => onComplete()} className="flex-1 py-4 bg-indigo-500 text-white font-black rounded-xl shadow-md border-b-4 border-indigo-700 active:translate-y-1 transition-all flex items-center justify-center gap-2">Next Lesson <ArrowRight size={18} /></button>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-full min-h-[600px] max-h-[90vh] flex flex-col bg-slate-200 font-sans overflow-hidden rounded-3xl border-4 border-slate-300 shadow-inner relative">
            ...
            {/* PROMINENT CENTRAL TIMER - Fully Visible and Opaque */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 pointer-events-none flex flex-col items-center justify-center">
                <div className={`px-6 py-2 md:px-8 md:py-3 rounded-full flex items-center gap-2 md:gap-3 shadow-2xl border-4 ${timeLeft <= 10 ? 'bg-red-100 border-red-500 text-red-600 animate-pulse' : (config.isHD ? 'bg-slate-900/90 backdrop-blur-md border-slate-700 text-white' : 'bg-slate-900 border-slate-700 text-white')}`}>
                    <Clock size={24} className="md:w-8 md:h-8" />
                    <span className="text-2xl md:text-4xl font-black leading-none">{Math.floor(timeLeft / 60)}:{(timeLeft % 60).toString().padStart(2, '0')}</span>
                </div>
            </div>
            ...
            <div className="flex-1 min-h-0 flex flex-col lg:flex-row gap-2 md:gap-4 p-2 md:p-4 z-10 relative bg-slate-300">
                <PlayerEngine 
                    ...
                    isMultiplayer={config.players === 2}
                    isHD={config.isHD}
                />
